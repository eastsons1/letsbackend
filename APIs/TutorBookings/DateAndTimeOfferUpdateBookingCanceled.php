									...
									else 
									{
										...
										$screen = '';
										 ...
										 if($notificationSent) {
											...
											$sql_Can = $conn->query("UPDATE tbl_payment SET cancel_alert_msg = '1' WHERE tutor_booking_process_id = '".$data['tutor_booking_process_id']."' and logged_in_user_id = '".$data['student_id']."' and amount = '0.00' and cancel_alert_msg = '0' ");
											
										
										
										/// Add Notification
										$chk_noti  = $conn->query("SELECT * FROM add_notifcations WHERE user_id_to_notification = '".$data['student_id']."' and title = '".$title."' and message = '".$body."' and source = 'FROM API - DateAndTimeOfferUpdateBookingCanceled' ");
										if(mysqli_num_rows($chk_noti)>0)
										{
											$del_No  = $conn->query("DELETE FROM add_notifcations WHERE user_id_to_notification = '".$data['student_id']."' and title = '".$title."' and message = '".$body."' and source = 'FROM API - DateAndTimeOfferUpdateBookingCanceled' ");
										
											$addN  = $conn->query("INSERT INTO add_notifcations SET user_id_to_notification = '".$data['student_id']."', title = '".$title."', message = '".$body."', source = 'FROM API - DateAndTimeOfferUpdateBookingCanceled', created_date = '".$create_date."' ");
										}
										else{
											$addN  = $conn->query("INSERT INTO add_notifcations SET user_id_to_notification = '".$data['student_id']."', title = '".$title."', message = '".$body."', source = 'FROM API - DateAndTimeOfferUpdateBookingCanceled', created_date = '".$create_date."' ");
