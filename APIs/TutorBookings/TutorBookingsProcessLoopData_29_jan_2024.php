...
function mime2ext($mime){
    ...
    return false;
			...
			foreach($Tutor_schedules as $value3) 
			{
				
				if($value3 !="" )
				{
					
					$arraySchedule[] = "('".$value3."')";
				}
			...
			if($conn->query($query))
			{
				...
				foreach ($Student_Level_Grade_Subjects as $entry) {
					...
					$query = $conn->query("INSERT INTO tutor_booking_process_Level_Grade_Subjects (ID, ALL_Subjects, Grade, Level, tutor_booking_process_id) VALUES ('$id', '$allSubjects', '$grade', '$level', '$getLastBooking_id_val')");
				...
				$Level_Grade_Subjects = $conn->query("UPDATE tutor_booking_process_Level_Grade_Subjects  SET tutor_booking_process_id = '".$getLastBooking_id['tutor_booking_process_id']."' where student_post_requirements_id = 0 "); 
				$scheduleTime_update = $conn->query("UPDATE tutor_booking_process_Schedules_Slot_Time SET tutor_booking_process_id = '".$getLastBooking_id['tutor_booking_process_id']."' where student_post_requirements_id = 0 ");  	
