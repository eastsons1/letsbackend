    ...
    return false;
			...
			// Extracting row by row
				...
				if($array["user_id"] !="" && $array["age"] !="")
				{
					...
					$arrayV[] = "('".$array["user_id"]."','".$img_name."','".$array["age"]."','".$array["gender"]."','".$array["nationality"]."','".$array["qualification"]."','".$array["name_of_school"]."','".$array["Course_Exam"]."','".$array["gra_year"]."','".$array["tutor_status"]."','".$array["tuition_type"]."','".$array["location"]."','".$array["postal_code"]."','".$array["travel_distance"]."','".$array["personal_statement"]."','".$tutor_code."')";				
			...
			if(mysqli_num_rows($chk_rec)>0)
			{
				...
				$query = "UPDATE user_tutor_info SET profile_image = '".$img_name."', age = '".$age."', gender = '".$gender."',nationality = '".$nationality."',qualification = '".$qualification."',name_of_school = '".$name_of_school."',Course_Exam = '".$Course_Exam."',gra_year = '".$gra_year."',tutor_status = '".$tutor_status."',tuition_type = '".$tuition_type."',location = '".$location."',postal_code = '".$postal_code."',travel_distance = '".$travel_distance."',personal_statement = '".$personal_statement."',tutor_code = '".$tutor_code."' where user_id = '".$user_id."' ";  
			
			}
			else			
			{	
				$query = "INSERT INTO `user_tutor_info` (user_id,profile_image,age,gender,nationality,qualification,name_of_school,Course_Exam,gra_year,tutor_status,tuition_type,location,postal_code,travel_distance,personal_statement,tutor_code) VALUES " . implode(', ', $arrayV);  
			}
			...
			if($conn->query($query)){
				...
				foreach ($HistoryAcademy_Array as $entry) {
					...
					$query = $conn->query("INSERT INTO complete_user_profile_history_academy (school, exam, subject, grade) VALUES ('$school', '$exam', '$subject', '$grade')");
				...
				foreach ($TutoringDetail_Array as $entry2) {
					...
					$query2 = $conn->query("INSERT INTO complete_user_profile_tutoring_detail (TutoringLevel, Tutoring_Grade, Tutoring_ALL_Subjects, Tutoring_Year, Tutoring_Month) VALUES ('$TutoringLevel', '$Tutoring_Grade', '$Tutoring_ALL_Subjects', '$Tutoring_Year', '$Tutoring_Month')");
