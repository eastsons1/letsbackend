			...
			if($conn->query($query)){
				...
				foreach($TutoringDetail_Array as $entry2) 
				{
					...
					if($entry2['TutoringLevel'] == "Secondary" )
					{
						...
						$query2 = $conn->query("INSERT INTO complete_user_profile_tutoring_detail (TutoringLevel, Tutoring_ALL_Subjects, Tutoring_Year, Tutoring_Month, user_id) VALUES ('$TutoringLevel', '$Tutoring_ALL_Subjects', '$Tutoring_Year', '$Tutoring_Month', '$user_id')");
						...
						foreach($entry2 as $entry3) 
						{
							...
							foreach($entry3 as $entry4) 
							{
								...
								if($AdmissionStreamResultID !="" && $AdmissionStreamResultID != 0)
								{
								...
								foreach($entry4['Stream'] as $streamData)
								{
									
									$AdmissionStreamquery2 = $conn->query("INSERT INTO complete_user_profile_tutoring_admission_stream (AdmissionStreamResultID, Stream, user_id) VALUES ('$AdmissionStreamResultID', '$streamData', '$user_id')");
					...
					if($entry2['TutoringLevel'] != "Secondary")
					{
						...
						//$TutoringDetail_del = $conn->query("delete from complete_user_profile_tutoring_detail where user_id = '".$user_id."' ");
					//echo "INSERT INTO complete_user_profile_tutoring_detail (TutoringLevel, AdmissionLevel, Tutoring_Grade, Tutoring_ALL_Subjects, Tutoring_Year, Tutoring_Month) VALUES ('$TutoringLevel', '$AdmissionLevel', '$Tutoring_Grade', '$Tutoring_ALL_Subjects', '$Tutoring_Year', '$Tutoring_Month')";
				...
				//$Update_admission_stream_detail = $conn->query("UPDATE complete_user_profile_tutoring_admission_level SET user_id = '".$user_id."' where user_id = 0 "); 	
				$Update_history_academy = $conn->query("UPDATE complete_user_profile_history_academy SET user_id = '".$user_id."' where user_id = 0 "); 	
				$Update_TutoringDetail = $conn->query("UPDATE complete_user_profile_tutoring_detail SET user_id = '".$user_id."' where user_id = 0 "); 	
