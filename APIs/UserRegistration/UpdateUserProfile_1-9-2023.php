...
function mime2ext($mime){
    ...
    return false;
			...
			if(mysqli_num_rows($chk_rec)>0)
			{
				...
				$query = "UPDATE user_tutor_info SET profile_image = '".$img_name."', age = '".$age."', gender = '".$gender."',nationality = '".$nationality."',qualification = '".$qualification."',name_of_school = '".$name_of_school."',Course_Exam = '".$Course_Exam."',OtherCourse_Exam = '".$OtherCourse_Exam."',gra_year = '".$gra_year."',tutor_status = '".$tutor_status."',tuition_type = '".$tuition_type."',location = '".$location."',postal_code = '".$postal_code."',travel_distance = '".$travel_distance."',tutor_tutoring_experience_years = '".$tutor_tutoring_experience_years."',tutor_tutoring_experience_months = '".$tutor_tutoring_experience_months."',personal_statement = '".$personal_statement."',tutor_code = '".$tutor_code."' where user_id = '".$user_id."' ";  
			...
			if($conn->query($query)){
				...
				/// Update User details
				$user_details_update = $conn->query("UPDATE user_info SET first_name = '".$first_name."', last_name = '".$last_name."', mobile = '".$mobile."', address1 = '".$address1."' WHERE user_id = '".$user_id."' ");
					...
					if($addStatus2==0)
					{
						$tutor_totoring_grade = $conn->query($aa="INSERT INTO `tutor_totoring_grade` (Tutoring_Grade,user_id) VALUES " . implode(', ', $arrayV4));  
					}
					if($addStatus3==0)
					{
					...
					if($addStatus5==0)
					{
						$tutor_totoring_level = $conn->query($aaa="INSERT INTO `tutor_totoring_levels` (Tutoring_Level,user_id) VALUES " . implode(', ', $arrayV7));  
					}
