...
///////
		...
		//die();
				...
				if($profile_image !="" )
				{
							...
							if(!in_array($ext, ['png', 'gif', 'jpeg', 'jpg', 'bmp', 'webp', 'tiff', 'svg', 'ico'])) {
					...
					if( $first_name!="" && $last_name!="" && $email!="" && $mobile!="" && $country_phone_code !="" && $device_token != "" && $device_type != "" )
					{
					  ...
					  if($email_chk == 1 && $mobile_chk == 1)	
					  {
						  ...
						  {	
							...
							$chk_temp_rec = $conn->query("SELECT * FROM user_info_temp WHERE email = '".$email."' ");
						
							if(mysqli_num_rows($chk_temp_rec)>0)
							{
								$del_sql = $conn->query("delete from user_info_temp WHERE email = '".$email."' ");
                               ...
                               // try {
                                  ...
                                  $body = '
                                   ...
                                   $mail->IsSMTP();                           // tell the class to use SMTP
                                    ...
                                    $mail->Host       = "eastsons.mytutors.moe"; // SMTP server  ////eastsons.mytutors.moe
		...
		if($_POST['OTP_EMAIL'] !="" )
		{
			...
			$check_otp = "SELECT * FROM user_info_temp WHERE OTP = '".$_POST['OTP_EMAIL']."' and email = '".$email."'  ";
		  ...
		  {
				 $update_sql = $conn->query("update user_info_temp set OTP_Validate ='1' where OTP = '".$_POST['OTP_EMAIL']."' and email = '".$email."'  ");
				 if($update_sql)
				 {
		...
		{
			$check_otp = "SELECT * FROM user_info_temp WHERE OTP = '".$_POST['OTP_EMAIL']."' and OTP_Validate ='1' and email = '".$email."' ";
			$check_otp_result = $conn->query($check_otp);
			$check_otp_exits = mysqli_num_rows($check_otp_result);
