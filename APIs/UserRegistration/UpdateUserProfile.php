...
function mime2ext($mime){
    ...
    return false;
			...
			{	
				 $query = "INSERT INTO `user_tutor_info` (user_id,profile_image,age,date_of_year,gender,nationality,flag,qualification,name_of_school,Course_Exam,gra_year,tutor_status,tuition_type,location,postal_code,travel_distance,personal_statement,lettitude,longitude,tutor_code,OtherCourse_Exam,stream) VALUES " . implode(', ', $arrayV);  
			}
			...
			if($conn->query($query))
			{
				...
				foreach($TutoringDetail_Array as $entry2) 
				{
					...
					if($entry2['TutoringLevel'] == "Secondary" )                           ///// For secondary postman send request will different.
					{
