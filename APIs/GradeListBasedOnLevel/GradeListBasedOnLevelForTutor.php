	...
	if($_POST['Level'] !="")
	{
				...
				if($_POST['Level']=="Secondary")
				{
					...
					while($grade = mysqli_fetch_assoc($grade_sql))
					{
						...
						if($grade['grade_name']=='Sec 5')
						{	
							
								$stream_sql = $conn->query("SELECT * FROM stream WHERE stream_name = 'NA' ");
								//$n=0;
								while($stream = mysqli_fetch_assoc($stream_sql))
								{
									
										$stream_array5[] = $stream;
									
								}
						
								$grade_array[] = 	array(
															'grade_name' => $grade['grade_name'],
															'streams_sec' => $stream_array5
			...
			{
				$resultData = array('status' => true, 'Grade_List_For_Tutor' => $grade_array);
			}
			else			
			{
