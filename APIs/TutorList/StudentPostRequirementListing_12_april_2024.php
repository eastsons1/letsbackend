				...
				if($Total_no_of_Tutor_Applied===0)
				{
					...
					{	
						...
						if($tutor_result['student_post_requirements_id']==$tutor_profile['student_post_requirements_id'])	
						{
								...
								$Apply_Tag_sql = $conn->query("SELECT * FROM student_post_requirements_Applied_by_tutor WHERE student_post_requirements_id = '".$tutor_profile['student_post_requirements_id']."' and tutor_login_id = '".$tutor_profile['tutor_login_id']."' ");
								
								$Apply_Tag = mysqli_fetch_array($Apply_Tag_sql);
								
								$Tutor_profile_img = mysqli_fetch_array($conn->query("SELECT profile_image FROM user_tutor_info WHERE user_id = '".$tutor_profile['tutor_login_id']."' "));
					...
					////  For tbl_Student_Level_Grade_Subjects_Post_Requirement grade
									...
									while($student_gradeList = mysqli_fetch_assoc($student_gradeSql))
									{
										...
										$student_grade[] = array('Level_Grade_Subjects_Post_Requirement_id ' => $student_gradeList['Level_Grade_Subjects_Post_Requirement_id'],
