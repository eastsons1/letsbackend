...
header('content-type:application/json');
		...
		if($_GET['student_id'] !="")
		{
			...
			$query = "SELECT * FROM student_post_requirements where logged_in_user_id = '".$_GET['student_id']."' order by student_post_requirements_id desc  ";
		   ...
		   ///// Tutor profile start
				...
				if($Total_no_of_Tutor_Applied===0)
				{
						...
						///////////////
								...
								////////////  For complete_user_profile_tutoring_detail
									...
									while($user_list_TD = mysqli_fetch_assoc($userTD))
									{
										...
										$Interested_Tutor_detail_Data = mysqli_fetch_array($conn->query("SELECT * FROM student_post_requirements_Applied_by_tutor appT INNER JOIN user_tutor_info tutor ON appT.tutor_login_id = tutor.user_id INNER JOIN user_info info ON tutor.user_id=info.user_id  WHERE appT.student_post_requirements_id  = '".$tutor_profile['student_post_requirements_id']."' and appT.tutor_login_id = '".$tutor_login_id."' "));
										
										
										$Apply_Tag_sql = $conn->query("SELECT * FROM student_post_requirements_Applied_by_tutor WHERE student_post_requirements_id = '".$tutor_profile['student_post_requirements_id']."' and tutor_login_id = '".$tutor_login_id."' ");

										$Apply_Tag = mysqli_fetch_array($Apply_Tag_sql);

										$Tutor_profile_img = mysqli_fetch_array($conn->query("SELECT profile_image FROM user_tutor_info WHERE user_id = '".$tutor_login_id."' "));
										...
										$interested_Tutor_arra[] = array(
															...
															'student_loggedIn_id' => $Interested_Tutor_detail_Data['student_loggedIn_id'],
					...
					$sql_bid = $conn->query("SELECT min(final_accepted_amount) FROM student_post_requirement_amount_negotiate WHERE student_post_requirement_id = '".$tutor_result['student_post_requirements_id']."' ");
					
					$sql_bid_data = mysqli_fetch_array($sql_bid);
					
					$min_bid_amount = $sql_bid_data['min(final_accepted_amount)']; 
					
					
					///////////
					
					$sql_update_amount = $conn->query("SELECT * FROM student_post_requirement_amount_negotiate WHERE student_post_requirement_id = '".$tutor_result['student_post_requirements_id']."' ");
