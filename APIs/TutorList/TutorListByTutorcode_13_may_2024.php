		...
		if($_GET['tutor_code'] !="")
		{
			...
			if($numrows > 0)
			{
				...
				while($tutor_result = mysqli_fetch_assoc($result))
				{
					...
					////////////
					$tutor_profile_data = mysqli_fetch_array($conn->query("SELECT * FROM user_tutor_info as Tprof INNER JOIN tutor_booking_process as TBookp ON Tprof.user_id = TBookp.tutor_id WHERE Tprof.tutor_code = '".$_GET['tutor_code']."' "));
					...
					$Response[] = array(
                      				...
                      				'tutor_id' => $tutor_profile_data['user_id'],
