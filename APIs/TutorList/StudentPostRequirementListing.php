		...
		if($_GET['student_id'] !="")
		{
				...
				//	{	
					...
					while($student_subject_res = mysqli_fetch_array($ss_query))
					{
						...
						if($student_subject_res['Level'] == "Secondary" && $student_subject_res['ALL_Subjects'] != "")
						{
							...
							//echo "SELECT * FROM student_post_requirements_streams WHERE student_post_requirements_id = '".$student_subject_res['student_post_requirements_id']."' ";
		   ...
		   ///// Tutor profile start
				...
				if($Total_no_of_Tutor_Applied===0)
				{
					...
					{	
						...
						///////////////
								...
								if($Interested_Tutor_No != 0 )						
								{
									...
									$RestOfInterested_Tutor = $conn->query("SELECT * FROM student_post_requirements_Applied_by_tutor WHERE student_post_requirements_id = '".$tutor_profile['student_post_requirements_id']."' and apply_tag = 'true' and booking_status <> 'booked' ");
									...
									$interested_Tutor_arra = array();
										...
										if($RestOfInterested_TutorID['tutor_login_id'] == $Interested_Tutor_detail_Data['tutor_login_id'])
										{	
									...
									//echo $Interested_Tutor_detail_Data['student_post_requirements_id'];
										...
										$Apply_Tag_sql = $conn->query("SELECT * FROM student_post_requirements_Applied_by_tutor WHERE student_post_requirements_id = '".$tutor_profile['student_post_requirements_id']."' and tutor_login_id = '".$Interested_Tutor_detail_Data['tutor_login_id']."' ");

										$Apply_Tag = mysqli_fetch_array($Apply_Tag_sql);
										
										
										
										$Tutor_profile_img = mysqli_fetch_array($conn->query("SELECT profile_image,flag FROM user_tutor_info WHERE user_id = '".$Interested_Tutor_detail_Data['tutor_login_id']."' "));
									...
									$tutor_arr[] = array(
															 'Interested_Tutor'=> $RestOfInterested_Tutor_No,
					...
					if(!empty($tutor_arr))
					{
						$InterestedTutorNo = $RestOfInterested_Tutor_No;
					}
					else{
						$InterestedTutorNo = '';
					}
