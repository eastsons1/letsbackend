...
header('content-type:application/json');
		...
		if($_GET['student_post_requirements_id'] !="")
		{
			...
			if($numrows > 0)
			{
				...
				$tutor_result = mysqli_fetch_assoc($result);
					...
					while($Tutor_Qualification_res = mysqli_fetch_array($TQ_query))
					{
						if($Tutor_Qualification_res['Tutor_Qualification'] != "")
						{
							$Tutor_Qualification[] = array(
														 ...
														 'student_post_requirements_id' => $Tutor_Qualification_res['student_post_requirements_id']
														 );
						}
					}
			...
			$resultData = array('status' => false, 'message' => 'Post requirements id can\'t be blank.');
		}	
