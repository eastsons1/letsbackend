...
function mime2ext($mime){
    ...
    return false;
			...
			foreach($array as $row => $value) {
				...
				if($value["student_id"] !="" && $value["tutor_id"] !="")
				{
				
					$arrayV[] = "('".$value["student_id"]."','".$value["tutor_id"]."','".$value["tution_type"]."','".$value["postal_code"]."','".$value["level"]."','".$value["grade"]."','".$value["negotiable_amount"]."','".$value["counter_amount"]."','".$value["fixed_amount"]."','".$value["booking_status"]."','".$value["booking_date"]."')";				
			...
			$chk_rec = $conn->query("select * from tbl_book_tutor_by_student where student_id = '".$student_idV."' and tutor_id = '".$tutor_idV."' ");
			...
			if(mysqli_num_rows($chk_rec)>0)
			{
				...
				$query = "INSERT INTO `tbl_book_tutor_by_student` (student_id,tutor_id,tution_type,postal_code,level,grade,negotiable_amount,counter_amount,fixed_amount,booking_status,booking_date) VALUES " . implode(', ', $arrayV);  
			}
			else			
			{	
			...
			if($conn->query($query)){
				...
				$student_subjects_update = $conn->query("UPDATE tbl_book_tutor_by_student_subjects SET booking_id = '".$getLastBooking_id['book_id']."' where booking_id = 0 ");  
					...
					{
						$resultData = array('status' => true, 'Message' => 'Student Added Successfully.');
					}
