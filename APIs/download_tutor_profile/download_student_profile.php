...
///Quatation Generate start
	...
	$student_info = mysqli_fetch_array(
					$conn->query("SELECT address1,first_name,last_name,mobile,profile_image,created_date FROM user_info WHERE user_id = '".$user_id."' ")
					);

//echo $student_info['profile_image'];

// Extract user details
	$student_name = ucfirst($student_info['first_name']) . ' ' . $student_info['last_name'];
...
//echo $student_info['profile_image'];		
	...
	else
	{
		...
		$FNC = substr($student_info['first_name'], 0, 1); 
		$LNC = substr($student_info['last_name'], 0, 1); 
...
<div id="main_box" style="width:100%; background:#2e5597; border-radius:17px;">
	...
	<div style="color:#ffffff; width: 100%; margin-bottom:3px; clear:both;  font-size: 20px;">'.$address.'</div>
