		...
		if($array['postal_code']!="" && $array['student_lat'] != "" && $array['student_long'] != "" )
		{
			...
			**/
			...
			else
			{
			 ...
			 if($user_rec_by_distance =="" && $stream_V == "" && $AdmissionLevel_V == "" && $TutoringLevel == "" && $qualification == "" && $nationality == "" && $subjects == "" && $grade == "" && $tutor_status == "" && $gender == "" && $sorting == "")
			 {
				 $All = 1;
			 }
			 else{
...
$check = "SELECT DISTINCT 
            ...
            complete_user_profile_tutoring_tutoring_subjects_detail.Tutoring_ALL_Subjects, 
          ...
          LEFT JOIN complete_user_profile_tutoring_grade_detail 
            ON complete_user_profile_tutoring_detail.user_id = complete_user_profile_tutoring_grade_detail.user_id 
          LEFT JOIN complete_user_profile_tutoring_tutoring_subjects_detail 
            ON complete_user_profile_tutoring_grade_detail.user_id = complete_user_profile_tutoring_tutoring_subjects_detail.user_id 
          LEFT JOIN complete_user_profile_tutoring_admission_stream 
            ON complete_user_profile_tutoring_tutoring_subjects_detail.user_id = complete_user_profile_tutoring_admission_stream.user_id 
          ...
          LEFT JOIN complete_user_profile_tutoring_admission_level 
            ON user_tutor_info.user_id = complete_user_profile_tutoring_admission_level.user_id WHERE ".$All.$user_rec_by_distance.$gender.$tutor_status.$nationality.$subjects.$TutoringLevel.$grade.$qualification.$stream_V.$AdmissionLevel_V." GROUP BY user_tutor_info.user_id ".$sorting;  
