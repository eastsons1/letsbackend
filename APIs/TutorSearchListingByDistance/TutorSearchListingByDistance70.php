    ...
    foreach ($array['subjects'] as $subject) {
        if (array_key_exists($subject, $subjectMappings)) {
            ...
            foreach ($subjectMappings[$subject] as $mappedSubject) {
                ...
                $mappedConditions[] = "s.Tutoring_ALL_Subjects = '{$safe_mapped_subject}'";
            }
            ...
            if (!empty($mappedConditions)) {
                $existsConditions[] = "
                    EXISTS (
                        SELECT 1 
                        FROM complete_user_profile_tutoring_tutoring_subjects_detail s 
            ...
            $existsConditions[] = "
                EXISTS (
                    ...
                    FROM complete_user_profile_tutoring_tutoring_subjects_detail s 
                    WHERE s.user_id = user_tutor_info.user_id 
                    AND s.Tutoring_ALL_Subjects = '{$safe_subject}'
                )
            ";
        }
    }
...
// Merge EXISTS conditions using AND to ensure all subjects are present
