...
header('content-type:application/json');
		...
		if($array['postal_code']!="" && $array['student_lat'] != "" && $array['student_long'] != "" )
		{
				...
				while($get_Data = mysqli_fetch_assoc($sql_Q))
				{
				    ...
				    if($get_Data['tuition_type'] != "")
					{
    					...
    					//echo $distance_value = distance($student_lat, $student_long, $lat, $long, "K") . " Kilometers<br>";
						 ...
						 //echo $distance_valueT.'=='.$travel_distance.'##';
			...
			{
				$AdmissionLevelString = "'" . implode("','", $array['AdmissionLevel']) . "'";
				
				$AdmissionLevel_V =  " and complete_user_profile_tutoring_admission_level.AdmissionLevel IN($AdmissionLevelString)";
			}
			else
			{
				$AdmissionLevel_V = "";
...
// Construct the final query
  $check = "SELECT DISTINCT user_tutor_info.user_id, 
            ...
            user_tutor_info.stream
				...
				LEFT JOIN complete_user_profile_tutoring_detail 
					...
					WHERE ".$All.$user_rec_by_distance.$gender.$tutor_status.$nationality.$subjects.$TutoringLevel.$grade.$qualification.$stream_V.$AdmissionLevel_V." GROUP BY user_tutor_info.user_id ".$sorting; 


			/**
				// Dynamically add INNER JOINs based on conditions
				if (!empty(trim($grade))) {	
					$check .= " INNER JOIN complete_user_profile_tutoring_grade_detail 
					ON user_tutor_info.user_id = complete_user_profile_tutoring_grade_detail.user_id";
				}

				if (!empty(trim($subjects))) {	
					$check .= " INNER JOIN complete_user_profile_tutoring_tutoring_subjects_detail 
					ON user_tutor_info.user_id = complete_user_profile_tutoring_tutoring_subjects_detail.user_id";
				}

				if (!empty(trim($stream_V))) {	
					$check .= " INNER JOIN complete_user_profile_tutoring_admission_stream 
					ON user_tutor_info.user_id = complete_user_profile_tutoring_admission_stream.user_id";
				}

				if (!empty(trim($AdmissionLevel_V))) {	
					$check .= " INNER JOIN complete_user_profile_tutoring_admission_level 
					ON user_tutor_info.user_id = complete_user_profile_tutoring_admission_level.user_id";
				}
				...
				if (!empty($All)) $whereClauses[] = $All;
				if (!empty($user_rec_by_distance)) $whereClauses[] = $user_rec_by_distance;
				if (!empty($gender)) $whereClauses[] = $gender;
				if (!empty($tutor_status)) $whereClauses[] = $tutor_status;
				if (!empty($nationality)) $whereClauses[] = $nationality;
				if (!empty($subjects)) $whereClauses[] = $subjects;
				if (!empty($TutoringLevel)) $whereClauses[] = $TutoringLevel;
				if (!empty($grade)) $whereClauses[] = $grade;
				if (!empty($qualification)) $whereClauses[] = $qualification;
				if (!empty($stream_V)) $whereClauses[] = $stream_V;
				if (!empty($AdmissionLevel_V)) $whereClauses[] = $AdmissionLevel_V;

				// Append WHERE conditions only if there are any
				if (!empty($whereClauses)) {
					$check .= " WHERE " . implode(" AND ", $whereClauses);
				}
